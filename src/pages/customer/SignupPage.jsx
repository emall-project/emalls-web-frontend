...
export default function SignupPage() {
  ...
  const submit = async (event) => {
    ...
    const missing = {};
    if (!form.fullName.trim()) missing.fullName = "الاسم الكامل مطلوب";
    if (!form.username.trim()) missing.username = "اسم المستخدم مطلوب";
    if (!form.number.trim()) missing.number = "رقم الهاتف مطلوب";
    if (!form.password) missing.password = "كلمة المرور مطلوبة";
    if (!form.age) missing.age = "العمر مطلوب";
    ...
    if (uploadingProfile) {
      setError("انتظر حتى ينتهي رفع الصورة");
      return;
    }
    ...
    try {
      ...
      const formError = buildApiFormError(requestError, FIELD_MAP, "فشل إنشاء الحساب");
      ...
      <div className="mx-auto max-w-3xl rounded-3xl border p-6 sm:p-8" style={{ background: "var(--gray-1)", borderColor: "var(--gray-a6)" }}>
        ...
        <form className="space-y-5" onSubmit={submit}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="رقم الهاتف *" error={fieldErrors.number || fieldErrors.prefix}>
            ...
            <Field label="رقم الهوية" error={fieldErrors.nationalIdNumber}>
          ...
          <MediaUuidField
            ...
            allowPicker={false}
            uploadMode="temp"
            error={fieldErrors.profilePictureUuid}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              ...
              {uploadingProfile ? "جاري رفع الصورة" : "إنشاء الحساب"}
            </button>
