...
const FEATURES = [
  "إدارة المتاجر والباعة داخل المول",
  "الإشراف على الإعلانات والحملات التسويقية",
  "متابعة المستخدمين والطلبات والتقارير",
];

export default function Login() {
  const navigate = useNavigate();
  const location = useLocation();
  const forgotPasswordHref = `/forgot-password?returnTo=${encodeURIComponent("/login")}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user?.role !== "ROLE_ADMIN") {
        auth.clear();
        setError("هذا الدخول مخصص للمشرفين فقط.");
        return;
      }
    ...
    <div dir="rtl" className="min-h-screen flex" style={{ background: "var(--gray-2)" }}>
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-4 py-10 sm:px-10">
        ...
        <div className="lg:hidden text-center mb-8">
          <div className="text-4xl font-bold tracking-wide bg-linear-to-l from-purple-600 to-indigo-600 bg-clip-text text-transparent">
            سوقنا
          </div>
          <span
        ...
        <div className="w-full max-w-90">
          ...
          <div className="hidden lg:block mb-7">
            ...
            <p className="text-sm mt-1" style={{ color: "var(--gray-10)" }}>
              سجّل الدخول للوصول إلى لوحة تحكم المشرف
            </p>
          </div>
